...
from config.db import db  # 导入数据库
...
class ResultCard(MDCard):
    def __init__(self, title, content, color=(0.98, 0.98, 0.98, 1), **kwargs):
        ...
        self.add_widget(MDLabel(text=title, font_style="Title", role="medium", size_hint_y=None, height="30dp",
...
class ResultScreen(Screen):
    ...
    def build_ui(self):
        ...
        self.status_label = ElderLabel(text="准备解读...", halign="center", size_hint_y=None, height="60dp",
                                       font_style="Headline", role="small")
        self.image_preview = Image(size_hint_y=None, height="200dp", allow_stretch=True, keep_ratio=True)
        self.result_container = MDBoxLayout(orientation='vertical', spacing="15dp", size_hint_y=None,
                                            adaptive_height=True)
    ...
    def _do_ocr(self, image_path):
        self.step_3_ai(ocr_engine.extract_text(image_path))
    ...
    def _do_ai(self, ocr_text):
        ...
        # 1. 渲染卡片
        self.result_container.add_widget(ResultCard("💡 核心结论", result['summary'], color=(0.8, 0.9, 1, 1)))
        if result['anomalies']:
        ...
        if self.current_image_path:
            db.add_history(result['summary'], result, self.current_image_path)
